'File Manager v.1.15 for PicoMiteVGA by JAVAVI (c)2025"
...
Print @(96,204)"[Shift]+[F3] - Hex List."
Print @(96,228)"[Alt]+[F1]...[F9] Keys - Alternate Conrol Functions."
Print @(96,252)"[Enter] Key - Entering a folder, running a file."
Print @(96,276)"[BS]&[Del] Keys - for editing lines in the input field."
Print @(96,300)"[Esc] Key - Escape from input & Exit to command prompt."
Color 0,c(14):Print @(520,84);"    ":Color 0,c(7):Print @(520,96);"    "
Do :Loop Until Inkey$<>""
Color c(15),c(0)
End Sub
/*Print @(96,180)"[Shift]+[F3] - Hex List."
  ...
  Do '* Main **************
    ...
    For i = 0 To bytesRead - 1 Step 16
        ...
        Else
           Print String$(13," ");
        EndIf
        Colour RGB(WHITE),GRAY'RGB(128,128,128)
    ...
    Select Case Key$
       ...
       Case INS$
          Print @(20*FW);
          Colour RGB(WHITE),RGB(RUST)
          Input " Enter New FileOffs: ",newOffset
          Colour RGB(WHITE),RGB(BLACK)
          If newOffset > fileSize Then
            startOffset = (((fileSize-1) \ pageSize) * pageSize)+1
            Print BELL$;
          ElseIf newOffset > 0 Then
            startOffset = ((newOffset \ pageSize) * pageSize)+1
          End If
       Case ESC$
...
For i = 15 To 0 Step-1
  ...
  Read cname$
  Colour RGB(BLACK),c
  If i < 6 Then Colour RGB(WHITE),c
  Print Str$(i,3)"    "+cname$+"   "
Next i
colors:
Data RGB(WHITE)," WHITE  ",RGB(YELLOW)," YELLOW "
Data RGB(LILAC)," LILAC  ",RGB(BROWN), " BROWN  "
Data RGB(FUCHSIA)," FUCHSIA",RGB(RUST)," RUST   "
Data RGB(MAGENTA)," MAGENTA",RGB(RED), " RED    "
Data RGB(CYAN)," CYAN   ",RGB(GREEN),  " GREEN  "
Data RGB(CERULEAN),"CERULEAN",RGB(MIDGREEN),"MIDGREEN"
Data RGB(COBALT)," COBALT ",RGB(MYRTLE)," MYRTLE "
Data RGB(BLUE)," BLUE   ",RGB(BLACK)," BLACK  "
Restore
End Sub
...
For j = 0 To 15
  For i = 1 To 16
    Colour RGB(BLUE),RGB(WHITE)
    If h=1 Then
      Print Format$(j*16+i-1,"%3g")" ";
    Else
     Print " "Hex$(j*16+i-1,2)" ";
    EndIf
    Colour RGB(WHITE),RGB(BLUE)
    If j*16+i-1 >=32 Then
      Print Chr$(j*16+i-1);
